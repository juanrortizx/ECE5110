...
\title{ Polynomial Interpolation for Modeling DC Motor Calibration }
...
\begin{abstract}
This article presents a practical application of the numerical modeling approach to polynomial interpolation. The study focuses on calibrating the steady-state response of a brushed DC motor used in manufacturing-robotic conveyor systems. Calibration data were collected across a voltage range from 0.5 V to 5.0 V, where the motor was allowed to reach steady-state at each configuration. The data set was processed using the Lagrange Interpolation Method, where the S-Curve was generated that passes through all measured points. This interpolation-based plant model forms the basis for tuning a PID controller tailored to the specific motor behavior. 
\end{abstract}
...
%\clearpage
\begin{figure}[H]
    \includegraphics[width=1.0\linewidth]{unit02/SCurve_Lagrange.png}
    \caption{S-Curve Plot for the Steady-State Response of a DC Motor}
    \label{fig:1}
\end{figure}
