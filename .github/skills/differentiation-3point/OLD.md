...
Then estimate acceleration by differentiating this interpolant twice using `DifferentiationTools.numerical_differentiation_3point` with the `central` method.
...
```python
self.assertAlmostEqual(abs(accel_est), 9.81, delta=0.15)
```
...
- return the estimated acceleration and enough metadata for reporting
...
### 7. `save_results(rows, article_results_dir: Path, run_index: int)`
...
### 10. `generate_article_images(rows, article_images_dir: Path)`
...
### 11. `generate_plots(tool: DifferentiationTools, plots_dir: Path)`
...
- a short gravity-check summary containing the estimated acceleration magnitude and its error relative to `9.81 m/s^2`
...
2. `test_gravity_from_interpolated_freefall_data`
   - fit the quadratic interpolant
   - differentiate twice using the library method
   - verify `abs(accel_est)` is within `0.15` of `9.81`
...
- [ ] the script defines the required free-fall position and time arrays
